...
class NetworkCheckApp(QMainWindow):
    ...
    def create_layout(self):
        ...
        self.continuous_ping_checkbox = QCheckBox(
            "Continuous Ping", self.central_widget
        )
        ...
        if hasattr(self, "timer") and self.timer.isActive():
            ...
            if not host or not interval.isdigit():
                QMessageBox.critical(
                    self,
                    "Error",
                    "Invalid input. Please enter a valid host and interval.",
                )
                return
    ...
    def ping(self):
        ...
        if response_time is not None:
            self.append_to_output(
                f"Ping response from {host}: {response_time:.2f} ms", "green"
            )
        else:
    ...
    def start_traceroute(self):
        ...
        if not host:
            QMessageBox.critical(
                self, "Error", "Invalid input. Please enter a valid host."
            )
            return
    ...
    def start_geolocation_lookup(self):
        ...
        try:
            ...
            self.append_to_output(
                f"Geolocation for {host}: Country - {country}", "green"
            )
        except:
    ...
    def append_to_output(self, text, color):
        ...
        self.output_window.verticalScrollBar().setValue(
            self.output_window.verticalScrollBar().maximum()
        )
    ...
    def save_log(self):
        ...
        file_name, _ = QFileDialog.getSaveFileName(
            self, "Save Log", "", "Text Files (*.txt);;All Files (*)", options=options
        )
...
if __name__ == "__main__":
    ...
    if sys.platform == "win32":
        fw_app_id = "fw.network.checkapp.1"
